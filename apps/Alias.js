...
export class Alias extends plugin {
    ...
    async addAlias(e) {
        ...
        const wiki = new Wiki();
        const entryData = await wiki.getEntry(char);
        if (!entryData.status) {
            return e.reply(`未找到名为 ${char} 的游戏内容，请检查名称再试`);
        }
    ...
    async aliasList(e) {
        const [, , char] = e.msg.match(this.rule[1].reg);
    ...
    async delAlias(e) {
        ...
        const [, , char, alias] = e.msg.match(this.rule[2].reg);
