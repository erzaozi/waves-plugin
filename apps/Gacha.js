...
export class Gacha extends plugin {
  constructor() {
    super({
      ...
      rule: [
        {
          reg: "^(～鸣潮|~鸣潮)(常驻(武器|角色)|限定(武器|角色))?抽卡(统计|分析|记录)([\\s\\S]*)$",
          fnc: "gachaCount",
        },
      ],
    });
  ...
  async gachaCount(e) {
    let message = e.msg.replace(
      /^(～鸣潮|~鸣潮)(常驻(武器|角色)|限定(武器|角色))?抽卡(统计|分析|记录)/,
      ""
    );
    ...
    if (message.startsWith("{")) {
      ...
      await e.reply(
        "未能解析成功，请在命令后面携带请求体或链接，各平台抽卡记录获取详细步骤请发送[~抽卡帮助]"
      );
      return true;
    }
    ...
    let cardPool = e.msg.includes("限定角色")
      ? { 1: "角色精准调谐" }
      : e.msg.includes("限定武器")
      ? { 2: "武器精准调谐" }
      : e.msg.includes("常驻角色")
  ...
  async dataFormat(data, type) {
    ...
    async function getCount4Star(startIdx, endIdx) {
      ...
      const sortedEntries = Object.entries(count4Star).map(
        async ([name, count]) => {
      ...
      return (await Promise.all(sortedEntries)).sort(
        (a, b) => b.count - a.count
      );
    }
    ...
    if (fiveStarIndexes.length > 0) {
      ...
      const promises = fiveStarIndexes.map(async (index, i) => {
        ...
        const endIdx =
          i + 1 < fiveStarIndexes.length ? fiveStarIndexes[i + 1] : data.length;
