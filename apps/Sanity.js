...
export class Sanity extends plugin {
  ...
  async querySanity(e) {
    ...
    if (deleteroleId.length) {
      let newAccountList = accountList.filter(
        (account) => !deleteroleId.includes(account.roleId)
      );
  ...
  async autoPush() {
    ...
    await Promise.all(
      autoPushList.map(async (user) => {
        ...
        let accountList =
          JSON.parse(await redis.get(`Yunzai:waves:users:${userId}`)) ||
          (await Config.getUserConfig(userId));
        ...
        for (let account of accountList) {
          ...
          const threshold =
            (await redis.get(`Yunzai:waves:sanity_threshold:${userId}`)) ||
            result.data.energyData.total;
          const isFull = result.data.energyData.cur >= threshold;
          if (isFull && !isPushed) {
            data.push({
              message: `漂泊者${result.data.roleName}(${result.data.roleId})，你的结晶波片已经恢复满了哦~`,
            });
        ...
        if (data.length) {
          if (data.length === 1) {
            if (groupId === "undefined") {
              ...
              await Bot[botId]
                ?.pickGroup(groupId)
                .sendMsg([segment.at(userId), data[0].message]);
            }
            return true;
          } else {
            if (groupId === "undefined") {
              await Bot[botId]
                ?.pickUser(userId)
                .sendMsg(
                  Bot.makeForwardMsg([{ message: `用户 ${userId}` }, ...data])
                );
            } else {
